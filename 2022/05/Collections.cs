...
namespace aoc
{
    public static class EnumberableCollectionsExtensions
    {
        ...
        public static IEnumerable<T> WhereNth<T>(this IEnumerable<T> items, int nStep){
            return items.Where((x, i) => i % nStep == 0);
        }

        public static IEnumerable<R> WhereNth<T, R>(this IEnumerable<T> items, int nStep, Func<T,int,R> resultSelector){
            
            return items
                .Select((x, i) => new Tuple<T,int>(x, i))
                .Where(t => t.Item2 % nStep == 0)
