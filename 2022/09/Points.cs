...
namespace aoc
{
    ...
    public static class Point2Extensions
    {
        ...
        public static Point2 MoveTowards(this Point2 p, Point2 t, int step = 1) =>
            new Point2(p.X + Math.Max(Math.Min(t.X - p.X, 1), -1) * step,
                p.Y + Math.Max(Math.Min(t.Y - p.Y, 1), -1) * step);
