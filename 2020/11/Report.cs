...
namespace aoc
{
    public static class Report
    {
        ...
        private static void PrintBanner(object result, string part, char bannerCharacter = '=')
        {
            ...
            Console.WriteLine(new string(bannerCharacter, 3) + $">> {result} <<" + new string(bannerCharacter, 3));
            stopwatch.Stop();
            Console.WriteLine(new string(bannerCharacter, 3) + $" Calculation took: {stopwatch.Elapsed} " + new string(bannerCharacter, 3));
            stopwatch.Restart();
